  ...
  return (
    <div style={{ background: 'var(--paper)', minHeight: '100vh' }}>
      <div style={{ background: 'var(--surface-solid)', borderBottom: '1.25px solid var(--border)', padding: '32px 0' }}>
        <Container size="lg">
          <PageHeader label="Admin" title={<span style={{ display: 'flex', alignItems: 'center', gap: 10 }}><ShieldCheck size={24} color="var(--primary)" />Review Queue</span>}
            subtitle={`${jobs.length} jobs pending AI verification`}
            actions={<Button variant="ghost" size="sm" onClick={fetchQueue} loading={loading}><RefreshCw size={13} />Refresh</Button>} />
        </Container>
      </div>
      <Container size="lg" style={{ padding: '32px 24px' }}>
        {loading
          ...
          : jobs.length === 0
            ? <EmptyState icon={<span style={{ fontSize: '2.5rem' }}>🎉</span>} title="All Caught Up!" body="No jobs pending review. Come back after the next scrape run." />
